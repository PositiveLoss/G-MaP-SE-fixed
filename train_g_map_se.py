...
def train(rank, a, h):
    ...
    if h.num_gpus > 1:
        ...
        discriminator = DistributedDataParallel(discriminator, device_ids=[rank]).to(
            device
        )
                ...
                if steps % a.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for batch in validation_loader:
                            ...
                            mag_g, pha_g, com_g = generator(
                                noisy_audio, noisy_mag, noisy_pha
                            )
                        ...
                        val_mag_err = val_mag_err_tot / validation_count
                        val_pha_err = val_pha_err_tot / validation_count
                        val_com_err = val_com_err_tot / validation_count
                        val_stft_err = val_stft_err_tot / validation_count
                        ...
                        sw.add_scalar(
                            "Validation/Consistency Loss", val_stft_err, steps
                        )

                    if (
                        epoch >= a.best_checkpoint_start_epoch
                        and val_pesq_score > best_pesq
                    ):
...
def main():
    ...
    parser.add_argument(
        "--input_training_file", default="VoiceBank+DEMAND/training.txt"
    )
