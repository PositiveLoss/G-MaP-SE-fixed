...
def compute_metrics(cleanFile, enhancedFile, Fs, path):
    ...
    # compute the SNRseg
    _, segsnr_dist = snr(data1, data2, sampling_rate1)
...
def llr(clean_speech, processed_speech, sample_rate):
    ...
    for frame_count in range(num_frames):
        ...
        _, _, A_processed = lpcoeff(processed_frame, P)
...
def stdft(x, N, K, N_fft):
    ...
    return np.transpose(x_stdft)[0:frames_size, :]
...
def taa_corr(x, y):
    ...
    return np.trace(np.matmul(xn, np.transpose(yn)))
